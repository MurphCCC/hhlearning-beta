...
if ($_SESSION['admin_mode'] === 'teacher') {
  ...
  // $sql = 'SELECT * FROM `courses` WHERE `student_id` = :sid';
    $sql = 'SELECT teachers.first_name AS tFirst, teachers.last_name AS tLast, courses.course_name AS cName, courses.id AS cId, courses.grade AS cGrade, courses.feedback AS cFeedback FROM courses INNER JOIN teachers on courses.teacher_id = teachers.teacher_id WHERE courses.student_id = :sid';
  $stmt = $db_con->prepare($sql);
  $stmt->bindParam(':sid', $_REQUEST['student_id'], PDO::PARAM_STR);
  $stmt->execute();
  $courses = $stmt->fetchAll(PDO::FETCH_ASSOC);
